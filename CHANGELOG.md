...
- Added a `close` method on the overlay service to let consumers close an overlay instead of using the deprecated `close` method on the overlay instance. [#143](https://github.com/blackbaud/skyux-core/pull/143)
- Fixed the overlay service to destroy the host component if all overlays are closed. [#143](https://github.com/blackbaud/skyux-core/pull/143)
